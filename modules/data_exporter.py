...
class DataExporter:
    def __init__(self, config):
        ...
        # asigura ca directorul de export exista
        os.makedirs(self.export_path, exist_ok=True)

    def export_data(self, violation_log, timestamp):
        """exporta datele in formatele specificate in configuratie"""
        results = {}
    ...
    def _export_to_csv(self, violation_log, timestamp):
        ...
        # creeaza dictionarul pentru statistici
        violation_types = {}
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            # mai intai scriem datele de sumar
            writer = csv.writer(csvfile)
            writer.writerow(["Raport generat", datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
            writer.writerow(["Total incalcari", str(len(violation_log))])
            writer.writerow([])

            # scriem statisticile pe tipuri de incalcari
            writer.writerow(["Tip incalcare", "Numar"])
            for violation_type, count in violation_types.items():
                writer.writerow([violation_type, count])
            writer.writerow([])

            # scriem inregistrarile detaliate ale incalcarilor
            writer.writerow(["Timestamp", "Incalcari"])
            for record in violation_log:
