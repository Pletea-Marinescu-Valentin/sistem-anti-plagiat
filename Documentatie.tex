...
Am utilizat două modele YOLOv8 separate, unul specializat pentru detectarea telefoanelor mobile și altul pentru detectarea ceasurilor inteligente. Această abordare a fost aleasă pentru a lucra eficient și pentru a elimina falsurile pozitive. În loc să reantrenăm un model unic pentru ambele categorii, am aplicat clasificări stricte bazate pe dimensiunile \textit{x-y} ale obiectelor detectate. Precizez că fiecare model YOLOv8 are un prag de încredere diferit, a carei valoare a fost stabilită în urma anumitor teste efectuate, iar aceasta se găsește în fișierul principal de configurație \texttt{config.json}.
...
\begin{minipage}{0.48\textwidth}
    \begin{figure}[H]
        \centering
        \includegraphics[height=7.5cm]{screenshots/s6.png}
        \caption{Cod pentru calculul raportului orizontal.}
        \label{fig:horizontal_ratio}
    \end{figure}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
    \begin{figure}[H]
        \centering
        \includegraphics[height=7.7cm]{screenshots/s7.png}
        \caption{Cod pentru calculul raportului vertical.}
        \label{fig:vertical_ratio}
    \end{figure}
\end{minipage}
...
\begin{enumerate}
    ...
    \item \textbf{Analiză post-eveniment:} Dezvoltarea unui modul avansat de analiză post-eveniment, care să permită revizuirea detaliată a sesiunilor de monitorizare, cu funcționalități precum căutarea pe baza marcajelor temporare sau generarea de grafice.
