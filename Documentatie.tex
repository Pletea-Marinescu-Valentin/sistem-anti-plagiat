...
\textbf{Explicație:} Diagrama secvențială ilustrează fluxul de evenimente din momentul în care utilizatorul pornește monitorizarea:
\begin{itemize}
    \item Utilizatorul inițiază monitorizarea prin interfața grafică (\texttt{AntiPlagiatGUI}).
    \item \texttt{VideoHandler} preia fluxul video și îl trimite către \texttt{GazeTracker} pentru analiza privirii și către \texttt{ObjectDetector} pentru detectarea obiectelor.
    \item \texttt{ViolationMonitor} monitorizează încălcările detectate de \texttt{GazeTracker}.
    \item \texttt{ReportGenerator} generează alerte și le trimite către interfața grafică pentru afișare.
    \item Utilizatorul vizualizează rezultatele în timp real.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        decision/.style={diamond, draw=black, fill=green!30, text centered, minimum width=4cm, minimum height=2cm},
        ...
        \node[decision, below of=analyze, yshift=-1.8cm] (violation) {Încălcare Detectată?};
        \node[process, right of=violation, xshift=3.5cm] (alert) {Generează Alertă};
        \node[process, below of=violation, yshift=-1.7cm] (save) {Salvează Frame};
        ...
        \draw[arrow] (violation) -- node[above] {Da} (alert);
        \draw[arrow] (violation) -- node[right] {Nu} (save);
        \draw[arrow] (alert) |- (save);
        \draw[arrow] (save) -- (update);
        \draw[arrow] (update) -- (stop);
        
        % Buclă de continuare
        \draw[arrow] (update.west) -- ++(-2.5,0) |- (capture.west);
    \end{tikzpicture}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        component/.style={rectangle, draw=black, fill=yellow!30, text centered, text width=3.5cm, minimum height=1.2cm, font=\sffamily\bfseries},
        ...
        \node[component] (video) at (-4.5,-3) {VideoHandler};
        \node[component] (face) at (4.5,-3) {FaceDetector};
        \node[component] (gaze) at (-4.5,-6) {GazeTracker};
        \node[component] (object) at (4.5,-6) {ObjectDetector};
        \node[component] (eye) at (-4.5,-9) {Eye / Pupil};
        \node[component] (data) at (4.5,-9) {DataExporter};
        \node[component] (report) at (0,-12) {ReportGenerator / ViolationMonitor};

        % Relații cu etichete mai vizibile
        \draw[relation] (gui) -- node[left, pos=0.3, text width=2cm, text centered, font=\sffamily] {Inițiază} (video);
        \draw[relation] (gui) -- node[right, pos=0.3, text width=2cm, text centered, font=\sffamily] {Controlează} (face);
        \draw[relation] (video) -- node[left, text width=2.5cm, text centered, font=\sffamily] {Analizează privirea} (gaze);
        \draw[relation] (video) -- node[above, sloped, font=\sffamily] {Procesare imagini} (object);
        \draw[relation] (gaze) -- node[left, font=\sffamily] {Detectează pupilele} (eye);
        \draw[relation] (object) -- node[left, pos=0.3, font=\sffamily] {Monitorizează} (report);
        \draw[relation] (gaze) -- node[left, pos=0.78, text width=2.5cm, text centered, font=\sffamily] {Transmite rezultate} (report);
        \draw[relation] (report) -- node[above, sloped, font=\sffamily] {Exportă date} (data);
        \draw[relation] (report) -| node[above, pos=0.9, font=\sffamily] {Raportare} (gui);
    \end{tikzpicture}
