...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[class, below=1.5cm of main] (sistem) {SistemAntiPlagiat\\\textit{Coordonator principal}\\\rule{3.5cm}{0.4pt}\\-config\\-video\_handler\\\rule{3.5cm}{0.4pt}\\+init()\\+start\_monitoring()\\+stop\_monitoring()};
        
        \node[class, below=1.8cm of sistem, xshift=-5.5cm] (gui) {AntiPlagiatGUI\\\textit{Interfață grafică}\\\rule{3.5cm}{0.4pt}\\-window\\-controls\\-timer\\\rule{3.5cm}{0.4pt}\\+init\_ui()\\+toggle\_monitoring()\\+export\_report()};
        
        \node[class, right=2.5cm of sistem] (video) {VideoHandler\\\textit{Procesare video}\\\rule{3.5cm}{0.4pt}\\-camera\\-frame\_buffer\\\rule{3.5cm}{0.4pt}\\+capture\_frame()\\+process\_frame()\\+save\_video()};
        
        \node[class, below right=2.5cm and -4.7cm of video] (object) {ObjectDetector\\\textit{Detectare obiecte}\\\rule{3.5cm}{0.4pt}\\-models\\-threshold\\\rule{3.5cm}{0.4pt}\\+detect\_objects()\\+load\_model()};
        
        \node[class, below=2.5cm of sistem] (face) {FaceDetector\\\textit{Detectare față}\\\rule{3.5cm}{0.4pt}\\-detector\\-predictor\\\rule{3.5cm}{0.4pt}\\+detect\_faces()\\+get\_landmarks()\\+analyze\_direction()};
        
        \node[class, below=1.6cm of face] (gaze) {GazeTracker\\\textit{Analiză privire}\\\rule{3.5cm}{0.4pt}\\-eye\_left\\-eye\_right\\\rule{3.5cm}{0.4pt}\\+analyze()\\+horizontal\_ratio()\\+vertical\_ratio()};
        
        \node[smallclass, below left=1.2cm and 1.3cm of gaze] (violation) {ViolationMonitor\\\textit{Monitorizare încălcări}\\\rule{2.8cm}{0.4pt}\\-violations\\-thresholds\\\rule{2.8cm}{0.4pt}\\+check\_violations()\\+log\_violation()};
        
        \node[smallclass, below right=1.3cm and 1.3cm of gaze] (eye) {Eye\\\textit{Analiză ochi}\\\rule{2.8cm}{0.4pt}\\-frame\\-landmarks\\\rule{2.8cm}{0.4pt}\\+isolate()\\+get\_pupil()};
        
        \node[class, below =1.4cm of eye] (pupil) {Pupil\\\textit{Analiză pupile}\\\rule{3.5cm}{0.4pt}\\-x\\-y\\-size\\\rule{3.5cm}{0.4pt}\\+detect()\\+get\_position()};
        
        \node[class, below right=-3.6cm and 1.8cm of violation] (report) {ReportGenerator\\\textit{Generare rapoarte}\\\rule{3.5cm}{0.4pt}\\-violations\\-timestamps\\\rule{3.5cm}{0.4pt}\\+generate\_html()\\+generate\_csv()};
        
        \node[class, below=1.5cm of report] (exporter) {DataExporter\\\textit{Export date}\\\rule{3.5cm}{0.4pt}\\-data\\-format\\\rule{3.5cm}{0.4pt}\\+export\_to\_csv()\\+export\_to\_json()};
...
\begin{figure}[H]
    \centering
        \begin{tikzpicture}[
                scale=0.7, % Reduced scale from 0.9 to 0.7
                transform shape,
                ...
                \node[activation, fill=gray!20] (video_act1) at (6,-2) [minimum height=0.8cm] {};
                ...
                % Mesaje inițiale
                \draw[message] (gui_act1) -- node[above, font=\sffamily\scriptsize] {start\_monitoring()} (sistem_act1);
                \draw[message] (sistem_act1) -- node[above, font=\sffamily\scriptsize] {initialize\_camera()} (video_act1);
                \draw[message] (video_act1) -- node[above, font=\sffamily\scriptsize] {detect\_faces()} (face_act1);
                \draw[message] (face_act1) -- node[above, font=\sffamily\scriptsize] {detect\_objects()} (object_act1);
                \draw[message] (object_act1) -- node[above, font=\sffamily\scriptsize] {check\_violations()} (violation_act1);
                
                % Buclă de procesare
                \draw[thick] (5.5,-4) rectangle (15.5,-8); % Reduced size
                \node[font=\sffamily\scriptsize] at (6,-3.8) {loop [while monitoring]};
                ...
                % Mesaje în buclă
                \draw[message] (video_act2.north east) -- node[above, font=\sffamily\scriptsize] {frame\_captured()} (face_act2.north west);
                \draw[message] (face_act2.north east) -- node[above, font=\sffamily\scriptsize] {analyze\_gaze()} (object_act2.north west);
                \draw[message] (object_act2.north east) -- node[above, font=\sffamily\scriptsize] {detect\_objects()} (violation_act2.north west);
                \draw[return] (violation_act2.south west) -- node[above, font=\sffamily\scriptsize] {update\_ui()} (gui_act2.south);   % Secvența de stop
                ...
                \draw[message] (gui_act3.north east) -- node[above, font=\sffamily\scriptsize] {stop\_monitoring()} (sistem_act2.north west);
                \draw[return] (sistem_act2.south west) -- node[below, font=\sffamily\scriptsize] {monitoring\_stopped()} (gui_act3.south east);
                
                % Note explicative - reduse și repoziționate
                \node[note] at (-1.2,-2) {Inițiere};
                \node[note] at (16.2,-5.5) {Verificare};
                \node[note] at (-1.2,-5.5) {Update UI};
